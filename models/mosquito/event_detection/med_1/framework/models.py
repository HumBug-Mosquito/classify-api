...
class MTRCNN_pad_half(nn.Module):
    def __init__(self, class_num, dropout, MC_dropout, batchnormal, mean, std):
        ...
        self.register_buffer("mel_mean", torch.tensor(mean).view(1, 1, 1, -1))
        self.register_buffer("mel_std",  torch.tensor(std).view(1, 1, 1, -1))
    ...
    def forward(self, wav):
        ...
        # feats_windowed = skimage.util.view_as_windows(feat.T, (win_size, mel_bins), step=step_size)
        # print(feats_windowed.shape)
        
        feats_windowed = self.torch_view_as_windows(
            feat, win_size=win_size, step=step_size
        )
